(ns eval.deps-try.fs
  "Copy of babashka/fs v0.4.19 (v0.5.20 would require bb >v1.3.188)"
   ...
   (let [exts (if win?
                (let [exts (or (:win-exts opts)
                  ...
                  (if (and ext (contains? (set exts) ext))
                    ...
                    exts))
                [nil])
         paths (or (:paths opts) (eval.deps-try.fs/exec-paths))
               ...
               fs (loop [exts exts
                    ...
                    (if (seq exts)
                      (let [ext (first exts)
                            ...
                            f (if (eval.deps-try.fs/relative? program)
                                (eval.deps-try.fs/path p program)
                                (eval.deps-try.fs/path program))]
